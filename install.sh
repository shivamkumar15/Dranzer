...
check_dependencies() {
    ...
    if command -v yay >/dev/null 2>&1; then
        ...
        PKG_MANAGER="pacman"
        printf '\n\033[1;33mWarning: No AUR helper (yay/paru) detected. Some packages might fail to install.\033[0m\n'
    ...
    for cmd in waybar rofi kitty hyprlock swaync playerctl cava pipewire wireplumber fastfetch cliphist git bash python3; do
    ...
    if [ -n "$PKG_MANAGER" ] && [ ${#install_pkgs[@]} -gt 0 ]; then
        ...
        case "$PKG_MANAGER" in
            pacman)
                ...
                ;;
            yay)
                yay -S --needed --noconfirm "${uniq_pkgs[@]}"
                ;;
            paru)
                paru -S --needed --noconfirm "${uniq_pkgs[@]}"
                ;;
            apt)
                sudo apt-get update
...
install_dir "$REPO_DIR/.config/kitty/themes" "$CONFIG_HOME/kitty/themes"
...
install_file "$REPO_DIR/.local/bin/bitbeast-control-center" "$BIN_DIR/bitbeast-control-center"
install_file "$REPO_DIR/.local/bin/waybar-mic.sh" "$BIN_DIR/waybar-mic.sh"
...
install_file "$REPO_DIR/.local/bin/bitbeast-workspace-picker" "$BIN_DIR/bitbeast-workspace-picker"
install_file "$REPO_DIR/.local/bin/bitbeast-hyprspace" "$BIN_DIR/bitbeast-hyprspace"
